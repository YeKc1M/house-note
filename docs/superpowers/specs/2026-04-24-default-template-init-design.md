...
### E2E Test — Full Instance Creation Flow with Default Templates

```gherkin
Given 应用首次启动，已自动初始化默认模板
When 用户进入「首页」页面
And 用户选择模板「小区模板」并新建顶层实例
And 用户输入实例名称为「华润二十四城」
And 用户填入「地址」为「成华区双庆路」
And 用户填入「建成年份」为「2015」
And 用户点击「保存实例」
Then 系统提示「实例创建成功」
And 实例列表中显示「华润二十四城」

When 用户点击进入「华润二十四城」详情页
And 用户在「房子」区域点击「新建子实例」
And 系统显示可选子模板「房子模板」
And 用户选择「房子模板」
...
And 用户点击「保存实例」
Then 系统提示「子实例创建成功」
And 在「华润二十四城」的「房子」下可看到子实例「7栋-1203」

When 用户点击进入「7栋-1203」详情页
And 用户在「客厅」区域点击「新建子实例」
And 用户选择「客厅模板」
And 用户输入实例名称为「客厅」
And 用户在「电视状态」填入「完好」
And 用户点击「保存实例」
Then 系统提示「子实例创建成功」
And 在「7栋-1203」的「客厅」下可看到子实例「客厅」

When 用户返回「7栋-1203」详情页
And 用户在「卧室」区域点击「新建子实例」
And 用户选择「卧室模板」
And 用户输入实例名称为「主卧」
And 用户在「床垫状态」填入「新床垫」
And 用户点击「保存实例」
Then 系统提示「子实例创建成功」
And 在「7栋-1203」的「卧室」下可看到子实例「主卧」

When 用户返回首页
And 用户点击「华润二十四城」卡片
Then 页面展示「7栋-1203」卡片
When 用户点击「7栋-1203」卡片
Then 页面展示「客厅」和「主卧」卡片
